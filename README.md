...
scripts in this table, which write the JSON in `docs/`. The manuscript figure
files themselves are not: the four schematics are drawio diagrams whose sources
are not in this repository, and the plotted figures were rendered by a separate
manuscript-side script. `scripts/make_phase11a_figures.py` produces an earlier,
overlapping set (`fig1_motivation`, `fig2_ablation`, `fig3_qualitative`,
`fig4_regimes`, `fig5_pareto`, `fig6_sensitivity`) rather than these exact
files. Treat the PNGs in `figures/` as published artifacts whose underlying
values are reproducible, not as build outputs of this tree.
