...
| `scripts/export_figure_data.py` | re-exports every figure's numbers out of `reports/` into `docs/figure-sources/tikz/data/`; `--check` verifies without writing |
...
**On the figures — this changed, and in the right direction.** All ten figures
in the manuscript are now TikZ, and their sources are tracked here under
[`docs/figure-sources/tikz/`](docs/figure-sources/tikz/). Earlier versions of
this README had to say four of eleven were reproducible and the schematics were
drawio exports with no source. That is no longer true.

**The `.tex` sources contain no numbers.** Everything measured is read from
`data/`, which `scripts/export_figure_data.py` writes out of `reports/` and
`docs/`. So a run that moves an input moves the figures with it, and a figure
cannot silently drift from the data behind it.

```bash
cd docs/figure-sources/tikz
make data     # re-export the numbers from reports/
make check    # verify the exporter's assertions without writing
make          # build all ten PDFs
make install  # copy them into paper/figures/
```

One caveat worth keeping: `make data` reads run records, and the per-origin
checkpoints behind `regime-band` (~2.6 MB of intermediates) are not tracked
here — regenerate them with `scripts/m5_sparse_scale_rule_run.py` first if you
want that one rebuilt from scratch.
...
The full write-up lives in [`paper/`](paper/) — `main.tex`, `references.bib`,
the 10 figures it references and the vendored Wiley class files — **27 pages**,
self-contained and buildable with:
...
The manuscript is under review; volume, pages and DOI follow on acceptance.

```bibtex
@article{scalerag2026,
  title   = {ScaleRAG: Retrieval Augmentation of Time Series Foundation Models
             under Scale Mismatch},
  author  = {Alam, Nidan and Sisty, Abu Jafar and Mobin, Md Iftekharul and
